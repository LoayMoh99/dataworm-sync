...
# 1. تعديل اسم الداتابيز الافتراضي إلى salah_gold
...
def generate_dim_date():
    ...
    # 2. تعديل الـ Truncate والـ Insert ليروحوا لـ salah_gold
    client.command("TRUNCATE TABLE salah_gold.dim_date")
    client.insert("dim_date", rows, column_names=columns)
    print(f"Loaded {len(rows)} rows into salah_gold.dim_date")
...
# 3. تعديل الـ DAG ID ليكون متناسق مع شغلك
with DAG(
    dag_id="01_salah_bronze_to_gold",
